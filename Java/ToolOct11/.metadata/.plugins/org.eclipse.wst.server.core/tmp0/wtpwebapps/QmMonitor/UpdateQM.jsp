<%@page import="com.cts.dnb.Application1.QMUtilityVO"%>
...
<title>Edit Queue-Manager</title>
</head>
...
QMUtilityVO vo = (QMUtilityVO)session.getAttribute("currentQMcon");
String oldQM = (String) session.getAttribute("oldqmName");
%>
<h1>Update Queue Manager</h1></div>
<form action="UpdateQM" method="post" onsubmit="return ValidateAddBroker()">
<input type="hidden" name="oldqmn" value=<%=oldQM %>>
<table id="adminbroker">
<tr>
<td>Environment<font color="Red">*</font>:</td>
<td><input type="text" name="alias" id="alias" value=<%=vo.getEnv() %>></td>
<td><font color=red><span id="span_alias"></span></font></td>
</tr>
<tr>
<td>Queue-Manager Name<font color="Red">*</font>:</td>
<td><input type="text" name="qm" id="qm" value=<%=vo.getCon().getQMname()%>></td>
<td><font color=red><span id="span_qm"></span></font></td>
</tr>
<tr>
<td>Host IP<font color="Red">*</font>:</td>
<td><input type="text" name="host" id="host" value=<%=vo.getCon().getQMhost()%>></td>
<td><font color=red><span id="span_host"></span></font></td>
</tr>
<tr>
<td>Port Number<font color="Red">*</font>:</td>
<td><input type="text" name="port" id="port" value=<%=vo.getCon().getQMport()%>></td>
<td><font color=red><span id="span_port"></span></font></td>
</tr>
<tr>
<td>Channel Name<font color="Red">*</font>:</td>
<td><input type="text" name="chaName" id="chaName" value=<%=vo.getCon().getQMchannel()%>></td>
<td><font color=red><span id="span_chaName"></span></font></td>
</tr>
</table>
<table align="center">
<tr>
<td><input type="submit" name="addBroker" id="addBroker" value="Validate and Update"></td>
...
<div id="right1">
<%if(session.getAttribute("QMAvailU")!=null ){
	Boolean qmavail = (Boolean)session.getAttribute("QMAvailU");
	if(!qmavail){
		session.removeAttribute("QMAvailU");
