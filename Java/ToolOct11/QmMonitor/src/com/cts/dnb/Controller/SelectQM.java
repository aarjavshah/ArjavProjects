...
public class SelectQM extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.debug("User "+session.getAttribute("username")+" selected "+qmName+" QueueManager");
		session.setAttribute("qmName", qmName);
		if(session.getAttribute("filteredQList")!=null){
			session.removeAttribute("filteredQList");
			response.sendRedirect(request.getContextPath()+"/FilterQueues");
