...
def SubscriptCorrector(src,temp,log):
	pathIn = src
	pathOut = temp
	LOG_FILE = log
			...
			while k>=0:
				...
				if line.strip().lower().startswith('set') and line.strip().split('=')[0].strip().endswith(' '+varN) and line.strip().split('=')[1].strip().split(';')[0].strip()=='0':
					...
					if line.strip().split(' ')[1]==varN and (int(float(line.strip().split(' ')[3].split(';')[0]))==0 or line.strip().split(' ')[3]=='0;'):
						...
						logger.debug("In "+src+" Correcting SET statement which was setting Index variable to 0.")
						commonFunc.printCorrection(src)
							...
							if flag:
								if (count == 4):
									...
									if initVal == 0:
										...
										logger.debug("In "+src+" Correcting Declaration statement intialized with 0 for Index variable.")
										commonFunc.printCorrection(src)
										line = tmp
